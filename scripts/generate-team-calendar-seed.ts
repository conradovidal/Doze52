...
const sources: Record<string, CalendarCatalogSource> = {
  brasileirao: { id: "cbf-brasileirao-2026", authority: "CBF", competition: "Campeonato Brasileiro Serie A", season: 2026, official_url: "https://www.cbf.com.br/futebol-brasileiro/tabelas/campeonato-brasileiro/serie-a/2026?documento=Tabela%20Detalhada", parser_key: "cbf", rollout_status: "shadow", freshness_hours: 28, last_checked_at: null, last_successful_at: null, last_error: null },
  copa: { id: "cbf-copa-do-brasil-2026", authority: "CBF", competition: "Copa do Brasil", season: 2026, official_url: "https://www.cbf.com.br/futebol-brasileiro/tabelas/copa-do-brasil/masculino/2026?documento=Tabela%20Detalhada", parser_key: "cbf", rollout_status: "shadow", freshness_hours: 28, last_checked_at: null, last_successful_at: null, last_error: null },
  libertadores: { id: "conmebol-libertadores-2026", authority: "CONMEBOL", competition: "CONMEBOL Libertadores", season: 2026, official_url: "https://gol.conmebol.com/libertadores/pt-br/tournament/15", parser_key: "conmebol", rollout_status: "shadow", freshness_hours: 28, last_checked_at: null, last_successful_at: null, last_error: null },
  sudamericana: { id: "conmebol-sudamericana-2026", authority: "CONMEBOL", competition: "CONMEBOL Sul-Americana", season: 2026, official_url: "https://gol.conmebol.com/sudamericana/pt-br/tournament/104", parser_key: "conmebol", rollout_status: "shadow", freshness_hours: 28, last_checked_at: null, last_successful_at: null, last_error: null },
};
...
const events = documents.flatMap(({ source, url, contentType }) =>
  parseOfficialSource(fetchOfficial(url), contentType, source)
    ...
    .map((event) => ({
      ...
      sourceUrl: source.parser_key === "cbf" ? source.official_url : url,
    }))
);
...
writeFileSync(resolve("lib/calendar-packs/brasileirao-2026-seed.json"), `${JSON.stringify({
  verifiedAt,
  sources: Object.values(sources).map((source) => ({ id: source.id, label: `${source.authority} — ${source.competition}`, url: source.official_url })),
  events: uniqueEvents,
}, null, 2)}\n`);
console.log(JSON.stringify({ clubs: clubs.length, events: uniqueEvents.length, bySource }, null, 2));
