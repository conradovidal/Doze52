...
export default function HomePage() {
  ...
  const mobileHabitsOnboardingNavLocked =
    !session?.user.id && (mobileHabitsOnboardingStep === "create_habit" ||
  ...
  const guidedToolbarNotice = React.useMemo<GuidedToolbarNotice | null>(() => {
    ...
    if (
      ...
      return {
        ...
        instruction:
          "Clique em Q1-Q4 para ver um trimestre ou em JAN-DEZ para ver um mês. Clique novamente no mês para voltar ao trimestre e no trimestre para voltar ao ano inteiro.",
        actionLabel: "Continuar",
        stepLabel,
      };
    }
    ...
    if (guidedOnboarding.step === "wrap_up_instruction") {
      const guidedProfileId = categories.find(
        (category) =>
          category.id === guidedOnboarding.dateCategoryId ||
          category.id === guidedOnboarding.periodCategoryId
      )?.profileId ?? selectedProfileIds[0];
      const categoryCount = guidedProfileId
        ? categories.filter((category) => category.profileId === guidedProfileId).length
        : 0;
      const remainingCategoryCount = Math.max(0, 3 - categoryCount);
      return {
        target: "wrap-up",
        title: "Veja o que você já construiu.",
        instruction: inlineEditModeActive
          ? remainingCategoryCount > 0
            ? `Escolha mais ${remainingCategoryCount} ${remainingCategoryCount === 1 ? "categoria" : "categorias"} para completar seu ano.`
            : "Suas três categorias estão prontas. Você pode trocar a última escolha antes de finalizar."
        ...
        actionDisabled: inlineEditModeActive && remainingCategoryCount > 0,
        stepLabel,
        categorySuggestions: getWrapUpCategorySuggestions(
          guidedOnboarding.context
        ),
        categorySuggestionCategoryIds:
          guidedOnboarding.wrapUpSuggestionCategoryIds,
      };
    }
    return null;
  }, [
  ...
  const handleGuidedToolbarAction = React.useCallback((
    ...
    if (
      ...
      const next = updateGuidedOnboarding({
        ...
        showHabit: !isAccountContinuityEnabled,
        finishAnnual: isAccountContinuityEnabled,
      });
  ...
  const handleDestinationSelect = React.useCallback(
    (destination: ProductDestinationId) => {
      ...
      if (destination === "annual" && isMobileCalendarUi === true) {
        ...
        if (
          !session?.user.id && !desktopTourInProgress &&
  ...
  // Continuação da jornada própria do mobile depois da Anual.
