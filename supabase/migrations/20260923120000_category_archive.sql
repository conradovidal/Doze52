-- Arquivamento de categorias: guarda quando foi arquivada e se os eventos
-- continuam aparecendo no calendário ('show') ou ficam escondidos ('hide').
-- Clientes antigos não enviam esses campos; o sync deles desarquiva (null).
begin;

alter table public.categories
  add column if not exists archived_at timestamptz,
  add column if not exists archive_display text
    check (archive_display is null or archive_display in ('show', 'hide'));
...
begin
  ...
  on conflict (id) do update set
    ...
    archive_display = excluded.archive_display,
    position = excluded.position,
    updated_at = now()
